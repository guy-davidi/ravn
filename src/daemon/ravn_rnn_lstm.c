...
int lstm_cell_init(lstm_cell_t* cell, int input_size, int hidden_size) {
	...
	if (!cell->h_prev || !cell->c_prev || !cell->h_curr || !cell->c_curr || !cell->f_gate ||
	    !cell->i_gate || !cell->c_candidate || !cell->o_gate) {
...
int lstm_cell_forward(lstm_cell_t* cell, const float* input) {
	...
	void matvec_mult(const float* matrix, const float* vector, float* result, int rows,
			 int cols) {
	...
	for (int i = 0; i < cell->hidden_size; i++) {
		cell->c_candidate[i] =
			tanh_activation(cell->c_candidate[i] + cell->h_curr[i] + cell->b_c[i]);
	}

	// Update cell state: C_t = f_t * C_{t-1} + i_t * C̃_t
	for (int i = 0; i < cell->hidden_size; i++) {
		cell->c_curr[i] =
			cell->f_gate[i] * cell->c_prev[i] + cell->i_gate[i] * cell->c_candidate[i];
	}
...
int ravn_model_init(ravn_model_t* model) {
	...
	if (dense_layer_init(&model->dense1, INPUT_SEQUENCE_LENGTH * INPUT_FEATURE_DIM,
			     DENSE1_OUTPUT_SIZE) != 0) {
		return -1;
	}
...
int ravn_model_load_weights(ravn_model_t* model, const float* all_weights) {
	...
	model->dense1.bias = (float*)&all_weights[INPUT_SEQUENCE_LENGTH * INPUT_FEATURE_DIM *
						  DENSE1_OUTPUT_SIZE];
	...
	model->lstm1.W_c =
		(float*)&all_weights[offset + 2 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE];
	model->lstm1.W_o =
		(float*)&all_weights[offset + 3 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE];
	model->lstm1.U_f =
		(float*)&all_weights[offset + 4 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE];
	model->lstm1.U_i =
		(float*)&all_weights[offset + 4 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE +
				     LSTM1_HIDDEN_SIZE * LSTM1_HIDDEN_SIZE];
	model->lstm1.U_c =
		(float*)&all_weights[offset + 4 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE +
				     2 * LSTM1_HIDDEN_SIZE * LSTM1_HIDDEN_SIZE];
	model->lstm1.U_o =
		(float*)&all_weights[offset + 4 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE +
				     3 * LSTM1_HIDDEN_SIZE * LSTM1_HIDDEN_SIZE];
	model->lstm1.b_f =
		(float*)&all_weights[offset + 4 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE +
				     4 * LSTM1_HIDDEN_SIZE * LSTM1_HIDDEN_SIZE];
	model->lstm1.b_i =
		(float*)&all_weights[offset + 4 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE +
				     4 * LSTM1_HIDDEN_SIZE * LSTM1_HIDDEN_SIZE + LSTM1_HIDDEN_SIZE];
	model->lstm1.b_c =
		(float*)&all_weights[offset + 4 * DENSE1_OUTPUT_SIZE * LSTM1_HIDDEN_SIZE +
				     4 * LSTM1_HIDDEN_SIZE * LSTM1_HIDDEN_SIZE +
				     2 * LSTM1_HIDDEN_SIZE];
	...
	offset =
		DENSE1_WEIGHT_COUNT + LSTM1_WEIGHT_COUNT + LSTM2_WEIGHT_COUNT + DENSE2_WEIGHT_COUNT;
...
float ravn_model_predict(ravn_model_t* model, const float* sequence, size_t sequence_length) {
	...
	if (dense_layer_forward(&model->dense1, model->sequence_buffer, model->dense1_output) !=
	    0) {
		return -1.0f;
	}
	...
	for (int t = 0; t < INPUT_SEQUENCE_LENGTH; t++) {
		if (lstm_cell_forward(&model->lstm1,
				      &model->dense1_output[t * DENSE1_OUTPUT_SIZE]) != 0) {
			return -1.0f;
		}
	}
...
int ravn_model_predict_class(ravn_model_t* model, const float* sequence, size_t sequence_length) {
	...
	if (model->final_output[0] > model->final_output[1] &&
	    model->final_output[0] > model->final_output[2]) {
		return 0; // Normal
