...
static logger_config_t g_logger_config = {.level = LOG_LEVEL_INFO,
					  .use_colors = 1,
					  .use_timestamps = 1,
					  .use_thread_id = 0,
					  .output_file = NULL,
...
int logger_init(log_level_t level, const char* log_file) {
	...
	LOG_INFO("Logger initialized - Level: %s, Output: %s", logger_level_name(level),
...
void logger_log(log_level_t level, const char* file, int line, const char* func, const char* format,
	...
	if (g_logger_config.use_thread_id) {
		pos += snprintf(log_buffer + pos, sizeof(log_buffer) - pos, "[TID:%s] ",
				thread_id_str);
	}

	// Log level with color
	pos += snprintf(log_buffer + pos, sizeof(log_buffer) - pos, "%s[%s]%s ", color_start,
			logger_level_name(level), color_end);
